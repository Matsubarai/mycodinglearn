...
复杂度不稳定, O(kE), 最多退化为Bellman-Ford算法的O(V*E)

将源点s丢入队列.

取出队首元素, 如果!visit, 遍历以该点为起点的边, 进行松弛操作.
...
Kruskal算法使用并查集维护最小生成树.

最小生成树指的是在联通的无向图中找到的联通所有顶点且边权最小的树.
...
拓扑排序(Topological Order)是指, 将一个有向无环图(Directed Acyclic Graph, DAG)进行排序进而得到一个有序的线性序列.
